...
while not((lambda ch: any(map(lambda a: all(map(lambda __: False if arr[a[0]*3+__] != a[1] else True, t)),([i,ch] for i in t))))('x' if var == -1 else 'o') or (lambda ch: any(map(lambda a: all(map(lambda __: False if arr[__*3+a[0]] != a[1] else True, t)),([i,ch] for i in t))))('x' if var == -1 else 'o') or (lambda ch: all(map(lambda _: arr[_*4] == ch, t)))('x' if var == -1 else 'o') or (lambda ch: all(map(lambda _: arr[_*2+2] == ch, t)))('x' if var == -1 else 'o') or not any(map(lambda c: c=='.', arr))):
  var = var * -1
